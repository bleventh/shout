...
def splitGrid(query):
	...
	for i in xrange(len(new_squares)):
		gridTable.add({ square : new_squares[i], boundaries[i][0], boundaries[i][1], boundaries[i][2], boundaries[i][3], users : user_squares[i] })-	gridTable.remove("quare", query.square)
...
def resizeGrid1(account_id, lon, lat):
	...
	if grid_density > 2*DESIRED_GRID_DENSITY:
		...
		joinGrid(query)
->>>>>>> 9fb5df67aa119f7a5728d5cd8cbcc63ae25efd29:psuedo/recursivegrid.py